...
label_continue:
	...
	{//EXIT ALGO
		state_t* exit_ch_begin = machine->current_state;

		if(common_parent != 0)
		{
			//in the case of being brother
			while(exit_ch_begin != common_parent)
			{
				event_t e = MAKE_NAKED_EVT(EVT_ID_EXIT);
				exit_ch_begin->handler(machine, e);
				exit_ch_begin = (state_t*)exit_ch_begin->parent;
			}
		}
		else
		{
			if(is_target_a_parent(machine, target))
			{
				while(exit_ch_begin != 0)
				{
					event_t e = MAKE_NAKED_EVT(EVT_ID_EXIT);
					exit_ch_begin->handler(machine, e);
					exit_ch_begin = (state_t*)exit_ch_begin->parent;

					if(exit_ch_begin == target)
					{
						exit_ch_begin->handler(machine, e);
						break;
					}
	...
	{//ENTER ALGO
		//search will start from bottom
		state_t* entry_ch_begin = target;

		//find begin of the chain.
		if(common_parent != 0)
		{
			while(entry_ch_begin->parent != common_parent)
			{
				entry_ch_begin = (state_t*)entry_ch_begin->parent;
			}
		}
		else
		{
			while((entry_ch_begin->parent != machine->current_state) &&
					(entry_ch_begin->parent != target) &&
					(entry_ch_begin->parent != 0))
			{
				entry_ch_begin = (state_t*)entry_ch_begin->parent;
			}
		}
		...
		while(entry_ch_begin != target)
		{
			machine->current_state = entry_ch_begin;
			...
			while(start->parent != entry_ch_begin)
			{
				start = (state_t*)start->parent;
			}

			entry_ch_begin = start;
		}
